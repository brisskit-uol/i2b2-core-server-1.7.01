...
public class DataMartLoaderBeanMDB implements MessageListener {
	...
	public void onMessage( Message inMessage ) {
		...
		try {
			if (inMessage instanceof TextMessage) {
				...
				String 
					fileSystemDefaultStorageResource = msg.getStringProperty(IROD_FILESYSTEM_STORAGE_RESOURCE);
				String 
					dsLookupDomainId = msg.getStringProperty(DS_LOOKUP_DOMAIN_ID);
				String 
					dsLookupProjectId = msg.getStringProperty(DS_LOOKUP_PROJECT_ID);
				String 
					dsLookupOwnerId = msg.getStringProperty(DS_LOOKUP_OWNER_ID);
				LoaderDAOFactoryHelper 
					daoFactoryHelper = new LoaderDAOFactoryHelper(	dsLookupDomainId
							                                     , dsLookupProjectId
							                                     , dsLookupOwnerId ) ;
				uploaderDaoFactory = daoFactoryHelper.getDAOFactory().getUpLoaderDAOFactory();

				sessionId = msg.getJMSCorrelationID();
				logger.info("MESSAGE BEAN: Message received: " + publishMessage);
				//
				...
				//
				// This comment originally said "four hours".
				// However, 43200 seconds is over 14 hours...
				String 
					transactionTimeoutVal = CRCLoaderUtil.getInstance().getProcessTransactionTimeout();
				int transactionTimeout = 43200;
				if (transactionTimeoutVal != null) {
					try {
						transactionTimeout = Integer.parseInt( transactionTimeoutVal.trim() ) ;
						log.debug("Transaction timeout from property file [" + transactionTimeout + "]");
				...
				// Process the file...
				utx.begin();
				processUploadMessage( uploaderDaoFactory
						            , userId
						            , uploadId
						            , localUploadFile
						            , publishMessage ) ;
			...
			if (throwable != null) {
				...
				try {
					utx.begin();
					updateUploadStatus( uploaderDaoFactory, uploadId, "ERROR", stringWriter.toString() ) ;
					utx.commit();
				} catch (Exception e) {
					log.error( "Failed to update 'error' status to upload status", e ) ;
				}
			}
			try {
				...
				t.printStackTrace();
				log.error("Error sending response ack [" + t.getMessage() + "]" ) ;
			}
		}
	}

	private String createLocalFile( int uploadId
			                      , String publishMessage
			                      , SecurityType i2b2SecurityType
			                      , String projectId
		...
		try {
			...
			DataListType dataListType = pubishDataReqType.getInputList().getDataFile();
			FRLocal testLocal = CRCLoaderUtil.getInstance().getFRBeanLocalHome().create();

			localUploadFile = testLocal.createClient( uploadId
					                                , dataListType
					                                , i2b2SecurityType
					                                , projectId
					                                , fileSystemDefaultStorageResource ) ;
			...
			e.printStackTrace();
			throw new I2B2Exception("Failed to create CRCFR-EJB :["	+ e.getMessage() + "]");
	...
	public int processUploadMessage( IUploaderDAOFactory uploaderDaoFactory
		...
		dmLoader.load( uploaderDaoFactory
				     , userId
				     , uploadId
				     , localUploadFile
				     , publishMessage);
