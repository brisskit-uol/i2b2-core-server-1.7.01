...
public class PidLoader extends AbstractDimensionLoader {
	...
	public int loadTempTable() throws I2B2Exception {
		...
		PidXmlDbLoader pidDbLoader = new PidXmlDbLoader( this.uploaderDaoFactory
				                                       , getOutputXmlFileName()
				                                       , getStagingTableName()
				                                       , getUploadId() ) ;
		pidDbLoader.doUpload();
		log.info("Uploaded " + getOutputXmlFileName() + "to staging table"	+ getStagingTableName());
		return pidDbLoader.getRowCount() ;
	}

	public int mergeTempTable() throws I2B2Exception {
		IPidDAO pidDAO = uploaderDaoFactory.getPidDAO();
		pidDAO.createPidFromTempTable( this.getStagingTableName(), getUploadId() ) ;
		log.info("Completed PID insert operation for staging table"	+ getStagingTableName());
		...
		setStatus.setLoadDate(new Date(System.currentTimeMillis()));
		UploadStatusDAOI uploadStatusDao = uploaderDaoFactory.getUploadStatusDAO();
		setStatus = uploadStatusDao.getUploadSetStatus(getUploadId(), UploadStatusDAO.PID_SET);
		setStatus.setLoadStatus("COMPLETED");
		setStatus.setLoadedRecord(pidDAO.getRecordCountByUploadId(getUploadId()));
	...
	public void load() throws I2B2Exception {
		...
		try {
			...
			tm.begin();
			PidXmlDbLoader 
				pidDbLoader = new PidXmlDbLoader( this.uploaderDaoFactory
						                        , getOutputXmlFileName()
						                        , getStagingTableName()
						                        , getUploadId() ) ;
			pidDbLoader.doUpload();
			...
			tm.begin();
			pidDAO.createPidFromTempTable( tempPatientMappingTableName, getUploadId());
			log.info("Completed PID insert operation for staging table" + getStagingTableName());
			tm.commit();
