...
public class ObservationFactXmlDbLoader implements StartElementListener {
	...
	public void doUpload() throws I2B2Exception {
		...
		IObservationFactDAO 
			observationFactDAO = uploaderDaoFactory.getObservationDAO();
		observationFactInsertHandle = 
			observationFactDAO.createObservationFactInserter(stagingObservationFactTableName);

		TypePullParser 
			parser = new TypePullParser(this, "observation", ObservationType.class, inputXmlFile);
		parser.doParsing();
	...
	public void process(Object observation) {
		...
		observationFactInsertHandle.insertObservationFact((ObservationType) observationType);
	}
