...
public class DataMartLoader implements IDataMartLoaderHelper {
	 ...
	 */
	public void load( IUploaderDAOFactory uploaderDaoFactory
			        , String userId
			        , int uploadId
			        , String localUploadFile
		...
		try {
			...
			try {
				JAXBElement<?> jaxbElement = jaxbUtil.unMashallFromString(publishMessage);
				publishType = (PublishDataRequestType) jaxbElement.getValue();
			} catch (JAXBUtilException jaxbEx) {
				throw new I2B2Exception("Error processing request message " + jaxbEx.getMessage(), jaxbEx);
			}

			String uploadFileName = localUploadFile;

			String inputLoadFileFormat = publishType.getInputList().getDataFile().getDataFormatType().toString();
			String sourceSystemCd = publishType.getInputList().getDataFile().getSourceSystemCd();
			boolean visitLoadFlag = (publishType.getLoadList().getLoadEventSet() != null) ? true : false;
			boolean observationFactLoadFlag = (publishType.getLoadList().getLoadObservationSet() != null) ? true : false;
			boolean appendFlag = false;
			if (observationFactLoadFlag) {
				appendFlag = publishType.getLoadList().getLoadObservationSet().isAppendFlag();
			}
			boolean pidLoadFlag = (publishType.getLoadList().getLoadPidSet() != null) ? true : false;
			boolean eidLoadFlag = (publishType.getLoadList().getLoadEidSet() != null) ? true : false;
			boolean patientLoadFlag = (publishType.getLoadList().getLoadPatientSet() != null) ? true : false;
			boolean conceptLoadFlag = (publishType.getLoadList().getLoadConceptSet() != null) ? true : false;
			boolean conceptDeleteExistingDataFlag = 
				(publishType.getLoadList().getLoadConceptSet() != null) ? publishType.getLoadList().getLoadConceptSet().isDeleteExistingData() : false;
			boolean observerLoadFlag = (publishType.getLoadList().getLoadObserverSet() != null) ? true : false;
			boolean observerDeleteExistingDataFlag = 
				(publishType.getLoadList().getLoadObserverSet() != null) ? publishType.getLoadList().getLoadObserverSet().isDeleteExistingData() : false;
			boolean stagingCleanUpFlag = publishType.getLoadList().isClearTempLoadTables();

			String encounterSource = "";
			String conceptCodePrefix = "";

			doNullCheck( uploadFileName, inputLoadFileFormat, sourceSystemCd, userId ) ;

			log.info("Starting to process loadfile :" + uploadFileName + "format " + inputLoadFileFormat + " userid " + userId);
			...
			UploadStatusDAOI uploadStatusDAO = uploaderDaoFactory.getUploadStatusDAO();
			if (pidLoadFlag) {
				...
				log.info( "Pid load complete " + uploadFileName + " uploadId" + uploadId ) ;
				if (stagingCleanUpFlag) {
					log.info("Droping temp table ["	+ pidLoader.getStagingTableName() + "]");
					uploadStatusDAO.dropTempTable(pidLoader.getStagingTableName());
				}
			}
			...
			if (eidLoadFlag) {
				...
				log.info("Eid load complete " + uploadFileName + " uploadId" + uploadId);
				if (stagingCleanUpFlag) {
					log.info("Droping temp table ["	+ eidLoader.getStagingTableName() + "]");
			...
			if (patientLoadFlag) {
				PatientLoader patientLoader = new PatientLoader( uploaderDaoFactory
						                                       , uploadFileName
						                                       , inputLoadFileFormat
						                                       , encounterSource
						                                       , sourceSystemCd
				...
				log.info("Patient load complete " + uploadFileName	+ " uploadId" + uploadId);
				if (stagingCleanUpFlag) {
					log.info("Droping temp table ["	+ patientLoader.getStagingTableName() + "]");
					uploadStatusDAO.dropTempTable(patientLoader.getStagingTableName());
				}
			}
			...
			if (visitLoadFlag) {
				...
				log.info("Visit load complete " + uploadFileName + " uploadId" + uploadId);
				if (stagingCleanUpFlag) {
					log.info("Droping temp table ["	+ visitLoader.getStagingTableName() + "]");
					uploadStatusDAO.dropTempTable(visitLoader.getStagingTableName());
				}
			}

			// check if concept set need to be loaded
			if (conceptLoadFlag) {
				log.info("Concept load started " + uploadFileName + " uploadId" + uploadId);

				ConceptLoader 
					conceptLoader = new ConceptLoader( uploaderDaoFactory
						                             , uploadFileName
						                             , inputLoadFileFormat
						                             , encounterSource
						                             , sourceSystemCd
						                             , conceptDeleteExistingDataFlag
						                             , uploadId ) ;
				performLoad(conceptLoader, UploadStatusDAO.CONCEPT_SET);
				log.info("Concept load complete " + uploadFileName	+ " uploadId" + uploadId);
				if (stagingCleanUpFlag) {
					log.info("Droping temp table ["	+ conceptLoader.getStagingTableName() + "]");
					uploadStatusDAO.dropTempTable(conceptLoader.getStagingTableName());
				}
			}

			// check if observer set need to be loaded
			if (observerLoadFlag) {
				log.info("Provider load started " + uploadFileName + " uploadId" + uploadId);

				ProviderLoader 
					providerLoader = new ProviderLoader( uploaderDaoFactory
							                           , uploadFileName
							                           , inputLoadFileFormat
							                           , encounterSource
							                           , sourceSystemCd
							                           , observerDeleteExistingDataFlag
							                           , uploadId ) ;
				performLoad(providerLoader, UploadStatusDAO.OBSERVER_SET);
				log.info("Provider load complete " + uploadFileName + " uploadId" + uploadId);
				if (stagingCleanUpFlag) {
					log.info("Droping temp table ["	+ providerLoader.getStagingTableName() + "]");
					uploadStatusDAO.dropTempTable(providerLoader.getStagingTableName());
				}
			}

			// check if observation fact need to be added
			if (observationFactLoadFlag) {
				ObservationFactLoader 
					obsFactLoader = new ObservationFactLoader( uploaderDaoFactory
							                                 , uploadFileName
							                                 , inputLoadFileFormat
							                                 , encounterSource
							                                 , conceptCodePrefix
				...
				log.info("ObservationFact load complete " + uploadFileName + " uploadId" + uploadId) ;
				if (stagingCleanUpFlag) {
					log.info("Droping temp table ["	+ obsFactLoader.getStagingTableName() + "]");
					uploadStatusDAO.dropTempTable(obsFactLoader.getStagingTableName());
				}
			...
			if (stagingCleanUpFlag) {
				try { // remove local input file File
					...
					log.error("Unable to delete the temp file [" + localUploadFile + " ]");
				}
			}
			...
			// rethrow exception
			throw new I2B2Exception("Error in Upload Process [" + ex.getMessage() + "]");
		}
	}
	...
	private void performLoad( AbstractDimensionLoader dimensionLoader, int setTypeId ) throws I2B2Exception {

		Throwable t = null;
		// setstatus started
		IUploaderDAOFactory uploadDaoFactory = dimensionLoader.getUploaderDaoFactory(); 

		try {

			utx.begin();
			UploadStatusDAOI uploadStatusDao = uploadDaoFactory.getUploadStatusDAO();
			...
			utx.begin();
			setStatus = uploadStatusDao.getUploadSetStatus(dimensionLoader.getUploadId(), setTypeId);
			...
			if (t != null) {
				...
				try { // check if exception thrown after the begin, if so
					...
					if (message != null) {
						setStatus.setMessage( message.substring( 0, ( message.length() > 3995) ? 3995 : message.length() ) ) ;
					}
					setStatus.setEndDate(new Date(System.currentTimeMillis()));
					uploadStatusDao.updateUploadSetStatus(setStatus);
					utx.commit();
