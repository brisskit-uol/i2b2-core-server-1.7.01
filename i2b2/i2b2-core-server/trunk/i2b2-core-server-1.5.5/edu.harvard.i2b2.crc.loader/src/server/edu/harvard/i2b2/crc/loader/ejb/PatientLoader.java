...
public class PatientLoader extends AbstractDimensionLoader {
	...
	public void load() throws I2B2Exception {
		try {
			...
			String tempPatientMappingTableName = "TEMP_PATIENT_MAP_" + getUploadId();
			setStagingTableName( tempPatientTableName ) ;

			IPatientDAO patientDAO = uploaderDaoFactory.getPatientDAO();
			patientDAO.createTempTable( getStagingTableName(), tempPatientMappingTableName ) ;
			log.info( "Created Patient staging table" + getStagingTableName() ) ;

			PatientXmlDbLoader 
				patientDbLoader = new PatientXmlDbLoader( uploaderDaoFactory
			...
			patientDAO.createPatientFromTempTable( getStagingTableName()
					                             ...
					                             , getUploadId() ) ;
			log.info("Completed Patient insert operation for staging table" + getStagingTableName());

			setStatus = uploadStatusDao.getUploadSetStatus( getUploadId(), UploadStatusDAO.PATIENT_SET ) ;
